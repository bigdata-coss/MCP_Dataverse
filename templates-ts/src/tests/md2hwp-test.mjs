/**
 * 마크다운을 HWP로 변환하는 라이브러리 테스트 (JavaScript 버전)
 */
  ...
  try {
    ...
    if (resultPath.endsWith('.json')) {
      ...
      if (jsonResult.sections.length > 0) {
        ...
        // 테이블 정보 확인
        const tables = section.content.filter((item) => 
          item.controls && item.controls.some((ctrl) => ctrl.type === 'table')
        );
        ...
        if (tables.length > 0) {
          ...
          // 첫 번째 테이블 정보
          const firstTable = tables[0].controls.find((ctrl) => ctrl.type === 'table');
          if (firstTable) {
            console.log("\n첫 번째 테이블 정보:");
            console.log(`- 행 수: ${firstTable.rows.length}`);
            console.log(`- 열 수: ${firstTable.rows[0].cells.length}`);
            console.log(`- 헤더 셀 내용: ${firstTable.rows[0].cells.map((cell) => cell.content).join(', ')}`);
          }
        }
      }
    }
