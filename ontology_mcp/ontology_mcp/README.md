...
Ontology MCP는 GraphDB의 SPARQL 엔드포인트를 Claude와 연결하는 Model Context Protocol (MCP) 서버입니다. 이 도구를 사용하면 Claude가 온톨로지 데이터를 쿼리하고 조작할 수 있습니다.
...
```bash
git clone https://github.com/bigdata-coss/MCP_Dataverse.git
cd MCP_Dataverse/ontology_mcp
```
...
```json
{
  "mcpServers": {
    "graphdb-mcp": {
      "command": "node",
      "args": ["/절대/경로/MCP_Dataverse/ontology_mcp/build"],
      "env": {
        "SPARQL_ENDPOINT": "http://localhost:7200",
        "SPARQL_DEFAULT_REPOSITORY": "schemaorg-current-https"
      },
      "disabled": false,
      "autoApprove": []
    }
...
> **중요**: `/절대/경로/MCP_Dataverse/ontology_mcp/build`를 프로젝트 빌드 디렉토리의 실제 절대 경로로 변경하세요.

3. Cursor/VSCode 재시작

## Claude Desktop에서 사용하기

Claude Desktop에서 다음 명령어를 사용하여 SPARQL 쿼리를 실행할 수 있습니다:
